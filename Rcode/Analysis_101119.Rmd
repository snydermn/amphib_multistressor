...
#View(liver2_t)
summary(liver2_t)
liver2_filename <- paste0(datadir,"liver2_t_out.csv")
write.csv(liver2_t, file=liver2_filename)
```
...
# https://rdrr.io/cran/muma/man/explore.data.html
# delete first row in excel of .csv 
explore.data(file=liver2_filename, scaling="a", scal=TRUE, normalize=TRUE, imputation=FALSE, imput="ImputType")
par( mfrow = c( 1, 2 ) )
Plot.pca(pcx=1, pcy=2, scaling="a", test.outlier=TRUE)
explore.data(file=liver2_filename, scaling="p", scal=TRUE, normalize=TRUE, imputation=FALSE, imput="ImputType")
```
...
liver3_filename <- paste0(datadir,"liver3_t_out.csv")
#pca
explore.data(file=liver3_filename, scaling="a", scal=TRUE, normalize=TRUE, imputation=FALSE, imput="ImputType")
par( mfrow = c( 1, 2 ) )
Plot.pca(pcx=1, pcy=2, scaling="p", test.outlier=TRUE)
explore.data(file=liver3_filename, scaling="p", scal=TRUE, normalize=TRUE, imputation=FALSE, imput="ImputType")
#plsda
plsda(scaling="p") 
...
View(acc4merge)
#plot of output
plot(acc4merge$no.features, acc4merge$out.acc, ylab="% Accuracy", xlab="Number of bins", main="control vs. pesticide", ylim=c(0,100))
...
#View(liver1_2twRFE)
rfe_ctrl_pest_filename <- paste0(datadir,"output/RFE_ctrlvspest.csv")
write.csv(liver1_2twRFE, rfe_ctrl_pest_filename)
```
...
View(acc4merge)
#plot of output
plot(acc4merge$no.features, acc4merge$out.acc, ylab="% Accuracy", xlab="Number of bins", main="control vs. bullfrog", ylim=c(0,100))
#finer scale plot
...
rfe_ctrl_bull_filename <- paste0(datadir,"output/RFE_ctrlvbullfrog.csv")
write.csv(liver1_3twRFE, rfe_ctrl_bull_filename)
```
...
#setwd("c:/git/PIP/GCMS_data_XCMS/output")
rfe75_filename <- paste0(datadir,"RFE_ctrlvspest_bull_75binrank.csv")
write.csv(yuck_exp, rfe75_filename)
```
...
View(acc4merge)
#plot of output
plot(acc4merge$no.features, acc4merge$out.acc, ylab="% Accuracy", xlab="Number of bins", main="control vs. pesticide+bullfrog", ylim=c(0,100))
...
View(liver1_4twRFE)
#setwd("c:/git/PIP/GCMS_data_XCMS/output")
liver1_filename <- paste0(datadir,"RFE_ctrlvpest_and_bullfrog_allbins.csv")
write.csv(liver1_4twRFE, liver1_filename)
```

#################################################################################
ADDING NEW CONTROL (CONTROL & BULLFROG ONLY VS. PESTICIDE & PESTICIDE + BULLFROG)

```{r get aligned scaled data}
liver1_class_filename <- paste0(datadir,"WorkDir_liver1/Preprocessing_Data_a/class.csv")
liver1_processedtable_filename <- paste0(datadir,"WorkDir_liver1/Preprocessing_Data_a/ProcessedTable.csv")
class <- read.csv(liver1_class_filename)
ProcessedTable <- read.csv(liver1_processedtable_filename)
...
m9
      [,1]  [,2]  [,3]  [,4]  [,5]  [,6]
[1,] 43.00 41.25 42.75 37.50 41.75 41.00
[2,] 44.25 38.00 43.75 40.25 40.75 40.75
[3,] 40.25 41.00 45.50 39.50 39.75 39.00
[4,] 40.00 42.25 42.75 42.50 41.50 39.25
[5,] 41.25 50.75 41.25 39.25 40.50 38.75
[6,] 50.75 48.75 41.50 42.75 41.75 40.75


RFE code for control & bullfrog vs. pesticide & pesticide + bullfrog
```{r}
#### other R implementation of RFE for SVM ####
svmrfeFeatureRanking = function(x,y){
  n = ncol(x)
  ...
  while(length(survivingFeaturesIndexes)>0){
    #train the support vector machine
    svmModel = svm(x[, survivingFeaturesIndexes], y, cost = 1e10, gamma=1.0e-10, cachesize=500,
                   scale=F, type="C-classification", kernel="radial" )
    #compute the weight vector
    w = t(svmModel$coefs)%*%svmModel$SV
    #compute ranking criteria
...
RFE and SVM with RFE for control & bullfrog vs. pesticide & pesticide + bullfrog. The RFE function has already been created above. 
```{r}
#prepare data for RFE
X<-liver1_3[,-1:-2]
X<-X[,-1585]
...
View(acc4merge)
#plot of output
plot(acc4merge$no.features, acc4merge$out.acc, ylab="% Accuracy", xlab="Number of bins", main="controls vs. pesticides", ylim=c(0,100))
#finer scale plot
no.features<-seq(1,200,by=20)
out.acc<-NULL
for (j in seq_along(no.features)){
